...
sed -e "s/\${PORT:-2233}/$PORT/g" \
    ...
    -e "s/\${TRANSLATOR_TYPE:-baidu}/baidu/g" \
    -e "s/\${BAIDU_TRANS_APPID:-}//g" \
    -e "s/\${BAIDU_TRANS_APIKEY:-}//g" \
    -e "s/\${OPENAI_API_BASE:-}//g" \
    -e "s/\${OPENAI_API_KEY:-}//g" \
    -e "s/\${OPENAI_MODEL:-gpt-3.5-turbo}/gpt-3.5-turbo/g" \
    -e "s/\${OPENAI_TIMEOUT:-30}/30/g" \
